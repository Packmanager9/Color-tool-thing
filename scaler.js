...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    let maxr = document.getElementById('maxr') //getting canvas from document
    let maxg = document.getElementById('maxg') //getting canvas from document
    let maxb = document.getElementById('maxb') //getting canvas from document
    let minr = document.getElementById('minr') //getting canvas from document
    let ming = document.getElementById('ming') //getting canvas from document
    let minb = document.getElementById('minb') //getting canvas from document
    let ab = document.getElementById('ab') //getting canvas from document
    let yes = document.getElementById('yes') //getting canvas from document
    let pressure = document.getElementById('pressure') //getting canvas from document
    yes.onclick = doupload
    ...
    fruits.src = 'fruitsprites11.png'
    canvas_context.drawImage(fruits,0,0)

    class Ascii {
        constructor(){
            this.colors = []
            this.letters = []
            this.brights = ['P', 'O', 'M', 'A', 'O']
            this.fruit =  0 //Math.floor(Math.random()*90)
            this.time = 0
        }
        segment(){     
            this.fruit++ // = Math.floor(Math.random()*90)
            this.colors = []
            this.letters = []
            let fnum = this.fruit   
            canvas.width = 100
            ...
            this.time +=.05
            let momheight = canvas.height + (Math.sin(((this.time * 1.3) + 3.14)) * 1.7)
            canvas_context.drawImage(pomaoimg, 0, 0, pomaoimg.width, pomaoimg.height, 0, 0 - (Math.sin(((this.time * 1.3) + 3.14)) * 1.9), canvas.width, momheight)


            // canvas_context.drawImage(fruits, (fnum % 10) * 186, (Math.floor(fnum / 10)) * 270, 186, 270, 0, 0, canvas.width, canvas.height)
            // canvas_context.drawImage(pomaoimg, 0,0, pomaoimg.width,pomaoimg.height, 0,0,50,50)
            const imageData = canvas_context.getImageData(0, 0, canvas.width, canvas.height);
            const data = imageData.data;
            for(let t = 0;t<data.length;t+=4){
                this.colors.push(`rgb(${data[t]},${data[t+1]},${data[t+2]})`)
                data[t]/=255
                data[t+1]/=255
                data[t+2]/=255
                let brightness = (data[t] + data[t+1] + data[t+2])/3
                // let brightness = ( ((0.299*data[t])*(0.299*data[t])) + (( 0.587*data[t+1])*( 0.587*data[t+1])) +(( 0.114*data[t+2])*( 0.114*data[t+2])))
                // console.log(brightness)
                brightness = Math.floor(brightness*4)
                // console.log(brightness)
                this.letters.push(this.brights[Math.floor(Math.random()*this.brights.length)])
            }
        }
        draw(){
            // console.log(this)
            ...
            for(let t = 0;t<this.colors.length;t++){
                    ...
                    canvas_context.fillText(this.brights[k%this.brights.length], ((t*7)%700)+((Math.random()-.5)*1), (10+(Math.floor(t/200)*14))+((Math.random()-.5)*2))
                        k++
                    }
            }
            ...
            // link.download = `ascmao${globalx}.png`
    ...
    let worldG = maxg.value
    let worldR = maxr.value
    let worldB = maxb.value
    let minimumR = minr.value
    let minimumG =  ming.value
    let minimumB =  minb.value
        canvas_context.clearRect(0, 0, canvas.width, canvas.height)  // refreshes the image
        // asc.segment()
        canvas_context.drawImage(pomaoimg, 0, 0, pomaoimg.width, pomaoimg.height, 0, 0, canvas.width, canvas.height)
        ...
        const data = imageData.data;
        for(let t = 0;t<data.length;t+=4){
            if(Math.floor(Math.floor(t/4)/canvas.width)%2==0){
            if(Math.floor(t/4)%2==0){
                if(ab.value == 1){
                    data[t] = (data[t] > minimumR ? worldR:0)
                    data[t+1] =(data[t+1] > minimumG ? worldG:0)
                    data[t+2] = (data[t+2] > minimumB ? worldB:0)
                }else{

                data[t] = (data[t] > minimumR ? data[t]:0)
                data[t+1] =(data[t+1] > minimumG ? data[t+1]:0)
                data[t+2] = (data[t+2] > minimumB ? data[t+2]:0)
                }
                }else{
                    data[t] = 0
                    data[t+1] = 0
    ...
    window.addEventListener('load', function() {
        document.querySelector('input[type="file"]').addEventListener('change', function() {
                pomaoimg.src = URL.createObjectURL(this.files[0]); // set src to blob url
